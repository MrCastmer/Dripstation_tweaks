...
/datum/component/tackler/proc/sack(mob/living/carbon/user, atom/hit)
	...
	var/datum/martial_art/A = H.check_block()
	if(H.check_shields(user, 0, user.name, attack_type = LEAP_ATTACK) || A)
		user.visible_message(span_danger("[user]'s tackle is blocked by [H], softening the effect!"), span_userdanger("Your tackle is blocked by [H], softening the effect!"), ignored_mobs = H)
		to_chat(H, span_userdanger("[H] blocks [user]'s tackle attempt, softening the effect!"))
		neutral_outcome(user, H, tackle_word) //Forces a neutral outcome so you're not screwed too much from being blocked while tackling
...
/datum/component/tackler/proc/rollTackle(mob/living/carbon/target)
	...
	var/obj/item/bodypart/defender_arm = target.get_active_hand()
	...
	if(ishuman(target))
		...
		if(HAS_TRAIT(tackle_target, TRAIT_TACKLING_TAILED_DEFENDER) && !el_tail)	//UH OH
			defense_mod -= 1
		if(el_tail && tackle_target.dna.species.is_wagging_tail()) // lizard tail wagging is robust and can swat away assailants!
			defense_mod += 2
	...
	//Arms contribute a great deal to potential tackling prowess and defense. Better arms = better bonus
	var/obj/item/bodypart/sacker_arm = sacker.get_active_hand()
	...
	if(ishuman(sacker))
		...
		var/datum/component/mood/mob_mood = human_sacker.GetComponent(/datum/component/mood)
		if(mob_mood) 
			switch(mob_mood.sanity_level)
				...
				if(SANITY_CRAZY)
					attack_mod += 10
					human_sacker.adjustStaminaLoss(70) //Crazy? I was crazy once.
...
/datum/component/tackler/proc/splat(mob/living/carbon/user, atom/hit)
	...
	switch(oopsie)
		...
		if(86 to 92)
			...
			user.adjustStaminaLoss(30, FALSE)
			user.adjustBruteLoss(30)
...
/datum/component/tackler/proc/splatWindow(mob/living/carbon/user, obj/structure/window/W)
	...
	if(W.type in list(/obj/structure/window, /obj/structure/window/fulltile, /obj/structure/window/unanchored, /obj/structure/window/fulltile/unanchored)) // boring unreinforced windows
		...
		user.adjustStaminaLoss(10 * speed, FALSE)
...
/datum/component/tackler/proc/checkObstacle(mob/living/carbon/owner)
	...
	owner.adjustBruteLoss(8 + messes.len, FALSE)
