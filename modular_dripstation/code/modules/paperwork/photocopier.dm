...
/obj/machinery/photocopier
	...
	var/busy = FALSE
	/// Variable needed to determine the selected category of forms on Photocopiernew.js
	var/category
...
/obj/machinery/photocopier/ui_interact(mob/user, datum/tgui/ui)
	...
	if(!ui)
		ui = new(user, src, "Photocopiernew")
		ui.open()
	...
	switch(action)
		...
		if("ai_photo")
			...
			if(!length(tempAI.aicamera.stored))
				balloon_alert(usr, "The clipboard is empty!")
				return FALSE
		...
		if("set_copies")
			...
			return TRUE
		// Changes the forms displayed on Photocopiernew.js when you switch categories
		if("choose_category")
...
/obj/machinery/photocopier/proc/check_busy(mob/user)
	if(busy)
		balloon_alert(user, "Photocopier is busy!")
		return TRUE
	return FALSE
...
/obj/machinery/photocopier/attackby(obj/item/object, mob/user, params)
	if(istype(object, /obj/item/paper) || istype(object, /obj/item/photo) || istype(object, /obj/item/documents))
		...
		if(toner_cartridge)
			balloon_alert(user, "There is already a cartridge inside!")
			return
...
/obj/machinery/photocopier/proc/insert_empty_paper(obj/item/paper/paper, mob/user)
	if(get_paper_count() >= MAX_PAPER_CAPACITY)
		balloon_alert(user, "Paper is at maximum capacity!")
		return
