...
/obj/item/forensics/swab/attack(mob/living/M, mob/user)
	...
	if(do_after(user,20,src))
		...
		var/target_dna
		var/target_gsr
		if(user.zone_sel.selecting == BODY_ZONE_PRECISE_MOUTH)
		...
		else if(user.zone_sel.selecting == BODY_ZONE_PRECISE_R_HAND || user.zone_sel.selecting == BODY_ZONE_PRECISE_L_HAND)
			var/has_hand
			var/obj/item/organ/external/O = H.get_bodypart(BODY_ZONE_R_ARM)
