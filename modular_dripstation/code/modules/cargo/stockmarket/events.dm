...
/datum/stockEvent/bankruptcy/proc/generateBankruptcyArticle()
	...
	if (prob(15))
		A.opinion = rand(-1, 1)
	var/article = "Another one might bite the dust: [company.current_trend > 0 ? "despite their positive trend" : "in line with their failing model"], [company.name] files for bankruptcy citing [pick(bankrupt_reason)]. The official representative of %country% has been asked to bail the company out, "
	if (!A.opinion)
...
/datum/stockEvent/arrest/proc/generateArrestArticle()
	...
	if (prob(15))
		A.opinion = rand(-1, 1)
	var/article = "[police] forces issued a statement that [tname], the [position] of [company.name], the %famous% %industrial% %company% was arrested %this_time%. The trial has been scheduled and the statement reports that the arrested individual is being charged with [offenses]. "
	if (!A.opinion)
