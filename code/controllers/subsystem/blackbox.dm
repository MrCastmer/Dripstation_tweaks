...
/datum/controller/subsystem/blackbox/proc/LogBroadcast(freq)
	...
	switch(freq)
		...
		if(FREQ_SYNDTEAM)
			record_feedback("tally", "radio_usage", 1, "syndteam")
		if(FREQ_TAIPAN)
			record_feedback("tally", "radio_usage", 1, "taipan")
		if(FREQ_ERT)
			record_feedback("tally", "radio_usage", 1, "ertradio")
		if(FREQ_SPECOPS)
			record_feedback("tally", "radio_usage", 1, "specopsradio")
		if(FREQ_LAW)
			record_feedback("tally", "radio_usage", 1, "lawradio")
		if(FREQ_AI_PRIVATE)
