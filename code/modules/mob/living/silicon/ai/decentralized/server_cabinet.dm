...
/obj/machinery/ai/server_cabinet/update_icon()
	...
	if(installed_racks.len > 0) 
		...
		SSvis_overlays.add_vis_overlay(src, icon, top_overlay, EMISSIVE_LAYER, EMISSIVE_PLANE, dir)
	if(installed_racks.len > 1) 
		...
		SSvis_overlays.add_vis_overlay(src, icon, bottom_overlay, EMISSIVE_LAYER, EMISSIVE_PLANE, dir)
	if(!(stat & (BROKEN|NOPOWER|EMPED)))
		var/mutable_appearance/on_overlay = mutable_appearance(icon, "expansion_bus_on")
		add_overlay(on_overlay)
		SSvis_overlays.add_vis_overlay(src, icon, on_overlay, EMISSIVE_LAYER, EMISSIVE_PLANE, dir)
		if(!valid_ticks)
			return
		if(installed_racks.len > 0)
			var/mutable_appearance/on_top_overlay = mutable_appearance(icon, "expansion_bus_top_on")
			add_overlay(on_top_overlay)
			SSvis_overlays.add_vis_overlay(src, icon, on_top_overlay, EMISSIVE_LAYER, EMISSIVE_PLANE, dir)
		if(installed_racks.len > 1)
			var/mutable_appearance/on_bottom_overlay = mutable_appearance(icon, "expansion_bus_bottom_on")
			add_overlay(on_bottom_overlay)
			SSvis_overlays.add_vis_overlay(src, icon, on_bottom_overlay, EMISSIVE_LAYER, EMISSIVE_PLANE, dir)
