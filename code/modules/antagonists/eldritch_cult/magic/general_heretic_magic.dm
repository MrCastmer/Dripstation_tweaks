...
//		return FALSE
	...
	if(isliving(victim))
		...
		if(iscarbon(victim))
			...
			if(IS_CULTIST(carbon_hit))										//Dripstation edit
				carbon_hit.AdjustKnockdown(0.5 SECONDS)						//Dripstation edit
				carbon_hit.adjust_confusion_up_to(1.5 SECONDS, 3 SECONDS)	//Dripstation edit
				carbon_hit.adjust_dizzy_up_to(1.5 SECONDS, 3 SECONDS)		//Dripstation edit
				...
				var/old_color = carbon_hit.color			//Dripstation edit
				carbon_hit.color = COLOR_CULT_RED			//Dripstation edit
				animate(carbon_hit, color = old_color, time = 4 SECONDS, easing = EASE_IN)	//Dripstation edit
				carbon_hit.mob_light(range = 1.5, power = 2.5, color = COLOR_CULT_RED, duration = 0.5 SECONDS)	//Dripstation edit
				playsound(carbon_hit, 'sound/magic/curse.ogg', 50, TRUE)					//Dripstation edit

				to_chat(caster, span_warning("An unholy force intervenes as you grasp [carbon_hit], absorbing most of the effects!"))			//Dripstation edit
				to_chat(carbon_hit, span_warning("As [caster] grasps you with eldritch forces, your blood magic absorbs most of the effects!"))	//Dripstation edit
				carbon_hit.balloon_alert_to_viewers("absorbed!")	//Dripstation edit
