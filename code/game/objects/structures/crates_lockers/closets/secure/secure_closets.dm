...
/obj/structure/closet/secure_closet/tool_interact(obj/item/W, mob/user)
	. = TRUE
	if(locked && broken == 0 && user.a_intent != INTENT_HARM)
		if(W.tool_behaviour == TOOL_SCREWDRIVER)
			...
			return TRUE
	else if(locked && broken == 3 && user.a_intent != INTENT_HARM)
		if(W.tool_behaviour == TOOL_WIRECUTTER)
			...
			return TRUE
	else if(locked && broken == 2 && user.a_intent != INTENT_HARM)
